...
test('health endpoint reports mock mode and series counts', async ({ request }) => {
  ...
  expect(j.market).toMatchObject({ on: false, source: 'off' })
  expect(r.headers()['cache-control']).toBe('no-store')
  // the market feed is switched off for the suite, so every number on the pages is the prototype's
  const m = await (await request.get('/api/market')).json()
  expect(m).toMatchObject({ ok: true, source: 'off', quotes: {} })
})
