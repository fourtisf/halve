    ...
    <div className="panel" style={{ marginTop: 16 }}>
      ...
      <div id="orders" className="orders">
        {orders.length === 0 ? (
          ...
          const action = o.status === 'filled' ? 'Claim' : o.status === 'partial' ? 'Close' : 'Cancel'
          return (
            <div className="row" key={o.key} data-status={o.status}>
              <div>
                <b>{what}</b>
                <span className={o.status === 'filled' ? 'g' : o.status === 'partial' ? 'y' : 'm'}>{o.status === 'filled' ? 'Filled' : o.status === 'partial' ? 'Filling' : 'Open'} · {holds}</span>
              </div>
