...
export function MarketsTable() {
  ...
  const go = (i: number) => router.push(`/app?s=${i}`)
  return (
          ...
          {rows.map(({ s, st, i }) => {
            ...
            return (
              <tr
                ...
                tabIndex={0}
                role="link"
                aria-label={`Open ${s.ticker} series`}
                onClick={() => go(i)}
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); go(i) } }}
              >
                <td><div className="tk"><i aria-hidden="true">{s.ticker.slice(0, 2)}</i><div><b>{s.ticker}</b><span>{s.name}</span></div></div></td>
                <td><span className="pill i">{s.issuer}</span></td>
                <td className="mono">{r ? f(st.divYield * 100, 1) + '%' : <Skel w={40} />}</td>
                <td className="mono g">{r ? f(st.fixedApy * 100, 1) + '%' : <Skel w={40} />}</td>
                <td className="mono"><span className="pill y">y{s.ticker}</span> {r ? f(st.ytPrice) : <Skel w={52} />}</td>
                <td>{monthYear(s.maturity)}</td>
                <td><span className="bar" aria-hidden="true"><i style={{ width: `${cap}%` }} /></span><span className="mono m">{cap}%</span></td>
