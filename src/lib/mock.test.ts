...
const schd = SERIES.find((s) => s.ticker === 'SCHD')!
const spy = SERIES.find((s) => s.ticker === 'SPY')!

describe('mock numbers reproduce prototype.html', () => {
  it('SCHD derived values (the demo headline series)', () => {
    const st = mockStats(schd)
    expect(st.ytPrice).toBe(0.0413)
    expect(st.fixedApy * 100).toBeCloseTo(8.28, 2)
    expect(st.leverage).toBeCloseTo(24.2, 1)
    expect(st.usdPrice).toBe(28.9)
    expect(st.tvlUsd).toBe(6.2e6)
    expect(st.accrued * 100).toBeCloseTo(2.6, 1)
    expect(st.events).toBe(4)
    expect(st.isSynced).toBe(true)
  })
  it('SPY is held with a pending special dividend', () => {
    expect(mockStats(spy).isSynced).toBe(false)
    const { rows, pending } = mockLedger('SPY')
    ...
    expect(rows).toHaveLength(2) // the pending row plus SPY's single classified event
  })
  it('ledger index compounds monthly', () => {
    const { rows } = mockLedger('SCHD')
